...
import { createReceiver, deleteReceiver, listReceivers, updateReceiver } from "../api/receivers";
...
export default function ReceiversTab() {
  ...
  const load = async () => {
    ...
    setMsg(null);
    try {
      const data = await listReceivers({ page: 1, limit: 50 });
      setItems(data?.data || []);
    } catch (e) {
      setMsg(e?.response?.data?.message || e.message || "Failed to load receivers");
    } finally {
      setLoading(false);
    }
  };
  ...
  const onSubmit = async () => {
    ...
    try {
      if (editId) {
        await updateReceiver(editId, form);
        setMsg("Updated receiver ✅");
      } else {
        await createReceiver(form);
        setMsg("Created receiver ✅");
      }
      setForm({ fullName: "", phone: "", address: "", country: "Nepal" });
      setEditId(null);
      await load();
    } catch (e) {
    ...
    <Stack spacing={2}>
      ...
      <Paper sx={{ p: 2, borderRadius: 2, bgcolor: "rgba(255,255,255,0.05)", color: "white" }}>
        <Typography fontWeight={800} sx={{ mb: 1 }}>
          {editId ? `Edit Receiver #${editId}` : "Create Receiver"}
        </Typography>
            ...
            >
              <Box>
                ...
                <Typography variant="body2" sx={{ opacity: 0.75 }}>
                  {r.phone || "No phone"} • {r.country || "-"} • #{r.id}
                </Typography>
              </Box>
